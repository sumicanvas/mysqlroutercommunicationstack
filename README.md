...
이전에는 이 변수에 `33061`과 같은 별도의 포트를 할당했습니다. 하지만 MySQL 8.0.27 이후, 그리고 8.4 + LTS 버전에서는 **Communication Stack**의 동작방식이 바뀌었습니다.

### A. 기존 방식: XCOM Stack (Legacy)
현재도 기본값은 이 값으로 설정되어 있습니다. 다만 mysql shell 을 이용해 클러스터를 생성 때 8.0.27 + 버전이면 기본값을 MYSQL 로 변경해 줍니다.
...
### B. 최신 방식: MySQL Stack (8.0.27 + & 8.4 +)
MySQL 8.0.27 + 부터 기본값으로 변경됨
...
## 3. Communication Stack 설정 로직

MySQL Server의 해당 변수의 기본값은 여전히 `XCOM`입니다. 하지만 클러스터를 구축할 때 사용하는 도구인 **MySQL Shell**에서 기본설정값을 변경합니다.
...
즉, 사용자가 별도의 옵션(`communicationStack: 'XCOM'`)을 주지 않으면, 최신 방식인 MySQL Stack을 기본값을 적용합니다. 이 때문에 사용자는 33061 포트를 설정한 하지 않아도, InnoDB Cluster는 3306 포트를 통해 정상적으로 동작하게 되는 것입니다.
...
* **MySQL 공식 매뉴얼 - Communication Stack 설명**
    * [MySQL 8.4 Reference Manual: Communication Stack for Connection Security Management](https://dev.mysql.com/doc/refman/8.4/en/group-replication-connection-security.html))


* **MySQL 시스템 변수 - group_replication_communication_stack**
    * [MySQL 8.4 Reference Manual: System varivables for group_replication](https://dev.mysql.com/doc/refman/8.4/en/group-replication-options.html#sysvar_group_replication_communication_stack)
